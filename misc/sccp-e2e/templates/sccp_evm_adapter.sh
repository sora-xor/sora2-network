...
if [[ "${action}" == "mint_verify" ]] && [[ -n "${SCCP_HUB_BUNDLE_JSON_PATH:-}" || -n "${SCCP_HUB_BUNDLE_NORITO_PATH:-}" || -n "${SCCP_HUB_BUNDLE_NORITO_HEX:-}" || -n "${SCCP_HUB_BUNDLE_SCALE_PATH:-}" || -n "${SCCP_HUB_BUNDLE_SCALE_HEX:-}" ]]; then
  output_file="$(mktemp)"
