...
benchmark::require_benchmark_binary() {
    local binary_path="$1"
...
benchmark::list_pallets() {
    ...
    "${binary_path}" benchmark pallet --list "${runtime_args[@]}" | tail -n +2 | cut -d',' -f1 | sort | uniq
}
...
benchmark::run_primary_target() {
    ...
    benchmark::run_target "${binary_path}" "${pallet_name}" "${output_path}" "${template_path}"
}
...
benchmark::run_all_pallet_targets() {
    ...
    echo "[+] Benchmarking ${#pallets[@]} pallets with runtime preset ${BENCHMARK_GENESIS_PRESET}"
