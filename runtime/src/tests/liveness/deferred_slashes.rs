...
fn deferred_repro_report(validators: &Validators) -> Perbill {
    ...
    assert_ok!(<crate::liveness::EquivocationReports as ReportOffence<
        AccountId,
        Identification,
        _,
...
#[test]
fn late_first_report_must_not_remain_in_an_already_drained_era() {
    ext().execute_with(|| {
...
#[test]
fn planned_era_must_not_unlock_funds_before_deferred_application() {
    ext().execute_with(|| {
        let validators = Validators::new();
        let account = &validators.accounts[0];
        let before = <Balances as Currency<AccountId>>::total_balance(account);
...
#[test]
fn deferred_application_must_use_the_original_offence_era() {
    ext().execute_with(|| {
        let validators = Validators::new();
        let account = &validators.accounts[0];
        let fraction = deferred_repro_report(&validators);
