...
- Focused `EthBridge` live-state rehearsal passed with:
  `SKIP_WASM_BUILD=1 REQUIRE_REMOTE=1 REMOTE_RPC_URL=wss://ws.mof.sora.org SNAP=/tmp/sora-ethbridge-focused-frame-key-pass-20260604.snap cargo test -p framenode-runtime --features try-runtime remote_eth_bridge_migration_rehearsal -- --exact --nocapture`
  This downloaded 228,562 `EthBridge::RequestStatuses` keys, 228,562 `EthBridge::Requests` keys, and the FRAME storage-version key for `EthBridge`; it completed with `1 passed; 0 failed` in 591.26s. Offline snapshot replay from `/tmp/sora-ethbridge-focused-frame-key-pass-20260604.snap` completed with `1 passed; 0 failed` in 16.05s.
- Filtered release-scope Executive rehearsal passed with:
  `SKIP_WASM_BUILD=1 SNAP=/tmp/sora-selected-plus-ethbridge-frame-key-20260604.snap REQUIRE_REMOTE=1 REMOTE_RPC_URL=wss://ws.mof.sora.org REMOTE_CHILD_TRIE=0 REMOTE_PALLETS=XorFee,Staking,Offences,Session,Grandpa,ImOnline,PoolXYK,PswapDistribution,VestedRewards,Identity,Farming,Kensetsu,Band,Polkamarkt,OracleProxy,BridgeInboundChannel,SubstrateBridgeInboundChannel,SubstrateBridgeOutboundChannel REMOTE_HASHED_PREFIXES=0x8e8f4d4777099074419cbb4597dac1d35c63ad4ba9b87d8a9938e57f2391a87b,0x8e8f4d4777099074419cbb4597dac1d3e973821932a99de497c2362ed9f9be34 REMOTE_HASHED_KEYS=0x8e8f4d4777099074419cbb4597dac1d34e7b9012096b41c4eb3aaf947f6ea429 ./misc/runtime_upgrade/run_remote_try_runtime.sh`
  This ran the normal `Executive::execute_on_runtime_upgrade()` path against selected live state for the 4.8.8 migration tuple, replacing the full `EthBridge` pallet prefix with the two maps touched by the v0-to-v3 migration plus the FRAME storage-version key. It completed with `1 passed; 0 failed` in 633.84s, and offline snapshot replay from `/tmp/sora-selected-plus-ethbridge-frame-key-20260604.snap` completed with `1 passed; 0 failed` in 1.12s.
