...
BRANCH_API_REF="$(jq -nr --arg v "${BRANCH}" '$v|@uri')"
...
gh api --method PUT "repos/${REPO}/branches/${BRANCH_API_REF}/protection" --input - <<<"${payload}" >/dev/null

gh api "repos/${REPO}/branches/${BRANCH_API_REF}/protection" |
  jq -r '
