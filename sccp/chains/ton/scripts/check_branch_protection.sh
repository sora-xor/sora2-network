...
if ! command -v jq >/dev/null 2>&1; then
  echo "check_branch_protection.sh requires jq in PATH" >&2
  exit 1
fi
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --branch)
      ...
      ;;
    --approvals)
      EXPECTED_APPROVALS="$(require_value --approvals "${2-}")"
      shift 2
      ;;
...
protection_json="$(gh api "repos/${REPO}/branches/${BRANCH_API_PATH}/protection")"
