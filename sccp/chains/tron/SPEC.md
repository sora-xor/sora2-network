...
- ETH/BSC/TRON: right-aligned 20-byte address.
- 20-byte account constraints on router:
  - high 12 bytes must be zero,
  - address must be non-zero.
- Solana / TON: project-specific 32-byte representation.

## BEEFY Validator Merkle Proofs (SORA -> TRON Verifier)

When importing a BEEFY commitment, signers prove membership in validator set root:
- leaf: `keccak256(bytes20(validator_signer_address))`
...
## TRON Burn Event Export Target

Canonical outbound `TRON -> SORA` burn export target:
...
Canonical extractor artifact schema:

`sccp-tron-burn-export/v1`

Consumers should enforce:
- log address equals the configured TRON router address,
...
- decoded `payload.source_domain == 5`,
- decoded `payload.dest_domain == 0`.

This repo defines the canonical TRON burn export surface only. It does not define or implement
the SORA-side trustless verification mechanism for `TRON -> SORA`.
