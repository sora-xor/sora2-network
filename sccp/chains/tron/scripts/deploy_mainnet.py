#!/usr/bin/env python3
"""Compile and deploy SCCP TRON contracts to mainnet.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compile + deploy SCCP contracts to TRON mainnet")
    ...
    p.add_argument("--skip-compile", action="store_true", help="Skip `npm run compile`")
...
def main() -> int:
    ...
    if not args.skip_compile:
        run(["npm", "run", "compile"], cwd=repo_root)
