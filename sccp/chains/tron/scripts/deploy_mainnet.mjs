...
const BOOLEAN_ARGS = new Set(['execute', 'resume']);
...
const KNOWN_ARGS = new Set([...BOOLEAN_ARGS, ...VALUE_ARGS]);
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (BOOLEAN_ARGS.has(key)) {
      if (nxt !== undefined && !nxt.startsWith('--')) {
        throw new Error(`Boolean flag --${key} does not take a value: ${nxt}`);
      }
      out[key] = true;
      continue;
    }
...
function hashParams(input) {
  const payload = stringifyJson(sortDeep(input));
...
function atomicWriteJson(path, value) {
  ...
  const tmp = `${path}.tmp`;
  writeFileSync(tmp, `${stringifyJson(value, 2)}\n`, 'utf8');
  renameSync(tmp, path);
}
...
async function main() {
  ...
  if (!execute) {
    ...
    console.log(stringifyJson(dryRun, 2));
    return;
  }
  ...
  writeFileSync(outPath, `${stringifyJson(output, 2)}\n`, 'utf8');

  console.log(stringifyJson(output, 2));
}
...
export { ensureStatePolicy, hashParams, parseArgs, stringifyJson };
