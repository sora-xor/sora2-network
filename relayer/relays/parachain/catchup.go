...
func (li *BeefyListener) buildMissedMessagePackages(
	...
	binary.LittleEndian.PutUint64(chainIdBytes, li.chainId)
	basicContractBytes, err := hex.DecodeString(li.config.Contracts.BasicInboundChannel)
	if err != nil {
		return nil, err
	}
	paraBasicNonceKey, err := types.CreateStorageKey(li.relaychainConn.Metadata(), "BasicOutboundChannel", "ChannelNonces", chainIdBytes, basicContractBytes)
	if err != nil {
		log.Error(err)
		return nil, err
	}
	...
	incentivizedContractBytes, err := hex.DecodeString(li.config.Contracts.IncentivizedInboundChannel)
	if err != nil {
		return nil, err
	}
	paraIncentivizedNonceKey, err := types.CreateStorageKey(li.relaychainConn.Metadata(), "IncentivizedOutboundChannel", "ChannelNonces", chainIdBytes, incentivizedContractBytes)
	if err != nil {
		log.Error(err)
		return nil, err
	}
	...
	for (!basicNonceFound || !incentivizedNonceFound) && currentBlockNumber != 0 {
		...
		for _, digestItem := range digestItems {
			if digestItem.IsCommitment {
				if digestItem.AsCommitment.NetworkId != li.chainId {
					continue
				}
