...
use ethash::{
    calc_dataset_item, get_cache_size, get_full_size, get_seedhash, make_cache, HASH_BYTES,
};
...
impl DatasetMerkleTreeCache {
    pub fn save(&self, path: impl AsRef<Path>) -> anyhow::Result<()> {
        ...
        let path = Self::cache_path(path, self.epoch as usize);
    ...
    pub fn load(path: impl AsRef<Path>, epoch: usize) -> anyhow::Result<Self> {
        let file = std::fs::OpenOptions::new()
            .read(true)
            .open(Self::cache_path(path, epoch))?;
        let res = serde_json::from_reader(file)?;
        Ok(res)
    }

    fn cache_path(path: impl AsRef<Path>, epoch: usize) -> PathBuf {
        path.as_ref().join(format!("{}.json", epoch))
    }
    ...
    pub async fn get_cache(
        ...
        let path = Self::cache_path(cache_dir.as_ref(), epoch);
        if !path.exists() {
        ...
        Self::load(cache_dir, epoch).context("load cache")
    }
}

pub fn make_dag(epoch: usize, dir: impl AsRef<Path>) -> anyhow::Result<()> {
    info!("Make dag for {} epoch in {:?}", epoch, dir.as_ref());
    let path = dag_path(epoch, &dir);
    if path.exists() {
    ...
    let mut dataset = vec![0; data_size as usize];
    let start = Instant::now();
...
pub fn dag_path(epoch: usize, dir: impl AsRef<Path>) -> PathBuf {
    let seed = get_seedhash(epoch);
    dir.as_ref()
        .join(format!("full-R23-{}", hex::encode(seed.as_bytes())))
}

pub fn make_dataset(dataset: &mut [u8], cache: &[u8]) {
    let count = AtomicUsize::new(0);
    let percent = dataset.len() / HASH_BYTES / 100;
    dataset
        ...
        .for_each(|(i, c)| {
            ...
            if current.checked_rem(percent).unwrap_or(0) == 0 {
                debug!("Make dataset: {}%", current / percent);
            }
        });
}
