mod test {
    ...
    use frame_system::{GenesisConfig, Pallet};
    use hex_literal::hex;
    ...
    fn liquidate_ok() {
        ...
        ext.execute_with(|| {
            ...
            let (treasury_reserve_dai, _, developer_amount_dai) =
                calculate_reserve_amounts(DAI, balance!(1000));
            ...
            // Developer / Authority
            assert_eq!(
                assets::Pallet::<Runtime>::free_balance(&DOT.into(), &get_authority_account())
                    .unwrap(),
                developer_amount_dot
            );

            assert_eq!(
                assets::Pallet::<Runtime>::free_balance(&DAI.into(), &get_authority_account())
                    .unwrap(),
    ...
    #[test]
    fn liquidate_via_offchain_worker_ok() {
        let mut offchain_ext = ExtBuilder::default().build();
        register_offchain_ext(&mut offchain_ext);

        offchain_ext.execute_with(|| {
            static_set_dex();
            init_exchange();
