...
impl<T: Config> OnChargeTransaction<T> for Pallet<T> {
    ...
    fn withdraw_fee(
        ...
        common::with_benchmark("xor-fee.withdraw_fee", || {
            if fee.is_zero() {
                return Ok(None);
            }
    ...
    fn correct_and_deposit_fee(
        ...
        common::with_benchmark("xor-fee.correct_and_deposit_fee", || {
            if let Some(paid) = already_withdrawn {
                ...
                if Technical::<T>::mint(&T::XorId::get(), &tech_account_id, xor_to_val).is_ok() {
                    let account_id =
                        Technical::<T>::tech_account_id_to_account_id(&tech_account_id)
