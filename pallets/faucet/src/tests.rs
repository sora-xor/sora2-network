...
fn transfer_passes_native_currency() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_eq!(
            Assets::free_balance(&AssetId::XOR, &account_id()).unwrap(),
            fixed!(50).into()
        );
        assert_eq!(
            Assets::free_balance(&AssetId::XOR, &alice()).unwrap(),
            fixed!(0).into()
        );
        assert_eq!(
            Assets::free_balance(&AssetId::XOR, &bob()).unwrap(),
            fixed!(100).into()
        );
    });
}
...
fn transfer_passes_multiple_assets() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_eq!(
            Assets::free_balance(&AssetId::VAL, &account_id()).unwrap(),
            fixed!(99, 57).into()
        );
        assert_eq!(
            Assets::free_balance(&AssetId::VAL, &alice()).unwrap(),
            fixed!(0).into()
        );
        assert_eq!(
            Assets::free_balance(&AssetId::VAL, &bob()).unwrap(),
            fixed!(50, 43).into()
        );
    });
}
...
fn transfer_fails_with_bad_origin() {
    ExtBuilder::build().execute_with(|| {
        assert_noop!(
            Module::transfer(Origin::root(), AssetId::XOR, bob(), fixed!(0, 5).into()),
