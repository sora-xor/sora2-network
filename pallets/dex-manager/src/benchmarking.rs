...
benchmarks! {
    ...
    verify {
        assert_last_event::<T>(Event::DEXInitialized(dex_id).into())
    }
    ...
    verify {
        assert_last_event::<T>(Event::FeeChanged(DEXId::Polkaswap.into(), 100).into())
    }
    ...
    verify {
        assert_last_event::<T>(Event::ProtocolFeeChanged(DEXId::Polkaswap.into(), 100).into())
    }
}
