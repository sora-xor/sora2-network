use crate::{
    self as pallet_polkamarket, AssetTransfer, ConditionId, OpengovProposalOf, PlazaIntegrationHook,
};
use frame_support::{
...
construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic,
    {
        ...
        Polkamarket: pallet_polkamarket::{Pallet, Call, Storage, Event<T>},
    }
);
...
impl pallet_polkamarket::Config for Test {
    type RuntimeEvent = RuntimeEvent;
    type WeightInfo = ();
    ...
    type OrderbookIntegration = pallet_polkamarket::OrderbookEventEmitter<Test>;
...
impl PlazaIntegrationHook<OpengovProposalOf<Test>> for MockPlazaIntegration {
    fn on_opengov_condition(condition_id: ConditionId, metadata: &OpengovProposalOf<Test>) {
        ...
        pallet_polkamarket::PolkadotPlazaBridge::<Test>::on_opengov_condition(
            condition_id,
            metadata,
        );
    }
