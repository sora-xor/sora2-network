...
fn test_exchange_tokens_1_should_pass() {
    ...
    ext.execute_with(|| {
        ...
        let result =
            LiquidityProxy::demo_function(DEX_A_ID, &GetBaseAssetId::get(), &DOT, Fixed::from(100))
                .expect("Failed to call demo function");
        // two dexes have liquidity source with this path, their prices are:
        assert_eq!(&result, &[Fixed::from_inner(136_851187324744592819)]);
        // only one dex for this path:
        let result =
            LiquidityProxy::demo_function(DEX_A_ID, &GetBaseAssetId::get(), &KSM, Fixed::from(100))
                .expect("Failed to call demo function");
        assert_eq!(&result, &[Fixed::from_inner(71_218100969694805079)]);
        let result = LiquidityProxy::demo_function(DEX_A_ID, &DOT, &KSM, Fixed::from(100))
            .expect("Failed to call demo function");
