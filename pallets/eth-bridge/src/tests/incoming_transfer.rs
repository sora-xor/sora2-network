...
#[test]
fn ocw_should_not_handle_disallowed_asset() {
    let mut builder = ExtBuilder::new();
    builder.add_network(
        vec![AssetConfig::Sidechain {
            id: XOR.into(),
    ...
    ext.execute_with(|| {
        ...
        let hash = crate::LoadToIncomingRequestHash::<Runtime>::get(net_id, &tx_hash);
        let err: DispatchError = match crate::Error::<Runtime>::AssetDisallowed.into() {
            DispatchError::Module { index, error, .. } => DispatchError::Module {
                index,
                error,
                message: None,
            },
...
#[test]
fn ocw_should_not_handle_failed_request_after_allowing_asset() {
    let mut builder = ExtBuilder::new();
    builder.add_network(
        vec![AssetConfig::Sidechain {
            id: XOR.into(),
...
#[test]
fn should_fail_incoming_transfer_with_disallowed_asset() {
    let (mut ext, state) = ExtBuilder::default().build();
    ext.execute_with(|| {
        let net_id = ETH_NETWORK_ID;
        assert_ok!(EthBridge::disallow_asset(
...
#[test]
fn should_include_incoming_transfer_with_allowed_asset() {
    let (mut ext, state) = ExtBuilder::default().build();
    ext.execute_with(|| {
        let net_id = ETH_NETWORK_ID;
        assert_ok!(EthBridge::disallow_asset(
