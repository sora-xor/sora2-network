...
use parity_scale_codec::alloc::collections::{HashMap, HashSet};
...
impl offchain::Externalities for TestOffchainExt {
    ...
    fn http_response_wait(
        ...
        let mut pending_ids: HashSet<_> = ids.iter().cloned().collect();
        let deadline = deadline.unwrap_or_default();
        let delay = (deadline.unix_millis() - 50).min(100);
        loop {
            {
                let state = self.0.read();
                for id in ids {
                    if pending_ids.contains(id) {
                        match state.requests.get(id) {
                            ...
                            None => {
                                ...
                                statuses.insert(id, RequestStatus::Invalid);
                            }
                            _ => {
                                pending_ids.remove(id);
                                statuses.insert(id, RequestStatus::Finished(200));
                            }
                        }
                    }
                }
            ...
            if self.timestamp() >= deadline {
                panic!(
                    "No `response` provided for requests with ids: {:?}",
                    pending_ids
                );
            }
