...
//! * Balances (a snapshot of balances, iroha account => val balance)
//! * Referrers (a snapshot of referrers, referral iroha account => referrer iroha account)
...
decl_storage! {
    trait Store for Module<T: Trait> as IrohaMigration {
        ...
        // Contains the account that VAL is mint with
        ...
        build(|config| {
            for (account_id, balance, referrer, threshold, public_keys) in &config.iroha_accounts {
                ...
                if public_keys.len() > 1 {
                    ...
                    PercentThresholds::insert(&account_id, Percent::from_rational_approximation(*threshold as usize, public_keys.len()));
                }
            }
        })
    }
        ...
        pub fn migrate(
            ...
            if key_count == 1 {
                ...
                let multisig_account_id = if approval_count == 1 {
                    ...
                    let multisig_account_id = multisig::Module::<T>::multi_account_id(
                        &who,
                        block_num,
                        nonce
                    );
