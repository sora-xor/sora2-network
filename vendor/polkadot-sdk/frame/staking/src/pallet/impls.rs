...
impl<T: Config> Pallet<T> {
    ...
    pub(super) fn do_migrate_currency(stash: &T::AccountId) -> DispatchResult {
        ...
        Self::deposit_event(Event::<T>::CurrencyMigrated {
            stash: stash.clone(),
            force_withdraw,
        });
        Ok(())
    ...
    pub fn on_offence(
        ...
        let active_era = {
            ...
            if active_era.is_none() {
                log!(
                    warn,
                    "🦹 on_offence: Active era not set -- not processing offence"
                );
        ...
        let active_era_start_session_index = ErasStartSessionIndex::<T>::get(active_era)
            .unwrap_or_else(|| {
                log!(
                    error,
                    "🦹 on_offence: start_session_index must be set for current_era"
                );
                0
            });
        add_db_reads_writes(1, 0);
        ...
        for (details, slash_fraction) in offenders.zip(slash_fractions) {
            ...
            if let Some(mut unapplied) = unapplied {
                ...
                if slash_defer_duration == 0 || slash_apply_era <= active_era {
                    ...
                    UnappliedSlashes::<T>::mutate(slash_apply_era, move |for_later| {
...
impl<T: Config> Pallet<T> {
    ...
    fn check_count() -> Result<(), TryRuntimeError> {
        ...
        let max_winners_per_page_bound = crate::MaxWinnersPerPageOf::<T::ElectionProvider>::get();
            ...
            ValidatorCount::<T>::get() <= max_validators_bound,
