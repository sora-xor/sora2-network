...
mod benchmarks {
    ...
    fn set_payee() -> Result<(), BenchmarkError> {
        ...
        #[extrinsic_call]
        _(
            RawOrigin::Signed(controller.clone()),
            RewardDestination::Account(controller.clone()),
        );

        assert_eq!(
            Payee::<T>::get(&stash),
            Some(RewardDestination::Account(controller))
        );
    ...
    fn cancel_deferred_slash(s: Linear<1, MAX_SLASHES>) {
        ...
        for _ in 0..MAX_SLASHES {
            unapplied_slashes.push(UnappliedSlash::<T::AccountId, BalanceOf<T>>::default_from(
                dummy(),
            ));
        }
        ...
        assert_eq!(
            UnappliedSlashes::<T>::get(&era).len(),
    ...
    fn restore_ledger() -> Result<(), BenchmarkError> {
        ...
        assert_eq!(
            Staking::<T>::inspect_bond_state(&stash),
